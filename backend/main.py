...
def _item(p):
    ...
    # product_groups: use the raw array from catalog JSON (e.g. ["iphone-17-pro"])
    raw_product_groups = p.get("product_groups") or []
    ...
    return {
        ...
        "product_group": base,
        "product_groups": raw_product_groups,
    }
...
def get_products(q: Optional[str]=None, type: Optional[str]=None, brand: Optional[str]=None,
                min_price: Optional[float]=None, max_price: Optional[float]=None,
                page: int=1, page_size: int=24, product_groups: Optional[str]=None):
    items = [_item(p) for p in ALL_PRODUCTS]
    # Filter by product_groups: match against the raw product_groups array in each catalog item
    # e.g. ?product_groups=iphone-17-pro returns only items whose product_groups list contains that value
    if product_groups:
        key = product_groups.strip().lower()
        items = [
            i for i in items
            if key in [pg.strip().lower() for pg in (i.get('product_groups') or [])]
        ]
    ...
    # build groups summary — when filtering by product_groups, scope variants to matched items only
    groups = {}
    source_for_groups = items if product_groups else af
    for it in source_for_groups:
        for pg in (it.get('product_groups') or []):
            g = pg.strip()
            if not g:
                continue
    ...
    groups_list = [{
        ...
        "count": len(v),
        "items": v,
        "hero_image": next((x["hero_image"] for x in v if x.get("hero_image")), ""),
        "price_from": min((x["price"] for x in v if x.get("price")), default=0),
